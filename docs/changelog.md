...
# 2026-09-06 — Public-sharing projection/route merged; deployed current main to Vercel production

- Issue #1 (V2-PS1 public-share projection) and issue #4 (V2-PS3 route/bridge) landed on `main` (`23a9c8b`, `3bc155e`, `b11da6a`). Independently re-verified before treating as done: 13/13 server tests, `cargo fmt --check`, `spacetime build`, 52/52 client tests, client lint, client production build, `git diff --check` — all clean on the committed `HEAD`.
- A planned Maincloud republish for the new schema was cancelled (no production change): the migration is additive at the source level, but the plan showed it would remove and recreate `my_rooms`, disconnecting every active client — the same view-recreation hazard documented earlier this session. Fixed as `b3f07ec`: preserved the deployed `PrivateRoomStatus`/`my_rooms.status` exactly as published, added the new v2 lifecycle state as a separate defaulted field exposed only through the new v2/public projections. Recorded as a decision in `docs/decisions.md` (2026-09-06 entry) so future schema work doesn't reintroduce the same break.
- Deployed the current `main` (`ef1c768`) to Vercel production via `vercel --prod` from `client/`; confirmed live at `pick-and-lock.vercel.app` (200, fresh build hash served).
- **Known limitation, not yet fixed**: the `/share/<id>` public-story route is wired into the client and deployed, but Maincloud has not been republished with the `shared_room_story` table (the cancelled migration above) — visiting `/share/<id>` in production will fail to subscribe until a republish happens. Core flows (create/join/chat/bot) are unaffected since they don't touch the new tables. Republishing needs the same owner confirmation every prior publish this session has required, and disconnects active clients, so it should happen in a deliberate window, not silently alongside this deploy.

# 2026-09-06 — Conversational poll-authoring spec written; Task 1 (context caching) merged

- Wrote `docs/superpowers/specs/2026-09-06-conversational-poll-authoring-design.md`: the bot notices decision intent without needing `@mention` (a cheap Tier 1 classifier gates a rare, context-rich Tier 2 call), understands the room it's in (title, current activities, budget/headcount/distance already stated), proposes ideas when a group has none, drafts multiple concurrent poll options through conversation (not one at a time — narrowing happens through the existing vote/lock mechanic, not chat), confirms before creating anything, and advises (never blocks) when a proposal conflicts with numbers the group already gave. Revised twice after review: once to support several competing options per room instead of one draft per room, once to add the reckless-decision advisory as its own final layer. Eight tasks total, each independently shippable; manual poll creation stays as a hidden fallback rather than being removed.
- Split the work in parallel across two workers on disjoint files to ship faster: Task 1 (bot-service context caching) went to an external collaborator on `server/bot-context-cache`, Task 4 (poll-draft schema, `server/spacetimedb/src/lib.rs`) started here in `.worktrees/bot-poll-draft-schema` — zero file overlap by design.
- Merged Task 1 (`d7a831d`): `service.ts` now caches full activity fields (price/minPeople/distanceKm/timeMinutes, previously only name), plan titles, and live member counts via a new `myRoomMembers` subscription. Independently re-verified before merging, not taken on the collaborator's self-report alone — their branch had forked before this session's spec/doc commits were pushed, so a naive diff against current `main` made untouched doc files look deleted; confirmed via `git merge-base` that the branch's own commits touched only `api/bot-service/`, then verified `tsc --noEmit` and 16/16 tests clean before merging.
- Built and merged Task 4 (`6db555f`): a new private `bot_poll_draft` table, `update_poll_draft`/`clear_poll_draft` reducers, and the `my_bot_poll_draft` view. Keyed by `(room_id, name)` with an auto-inc id rather than one row per room, so several competing named options can be drafted concurrently — a design correction made to the spec itself after review caught that the original one-draft-per-room shape couldn't represent a group naming several options before narrowing via voting. `cargo fmt`, 17/17 server tests, `spacetime build`, regenerated bindings, 52/52 client tests, lint, and build all clean before merging.
- Opened GitHub issues #31-#36 for the remaining six spec tasks and handed them to the external collaborator as one consolidated prompt (rather than one issue at a time) so they can work through independent tasks in parallel, gated on pulling `bot_poll_draft` from `main` before starting the two tasks (#35, #36) that depend on it.

# 2026-09-06 — Freeze-window push: Tasks 2/5/6/7 merged, published, and deployed live

- Owner supplied the hackathon freeze deadline (8:00 AM IST) mid-session; reprioritized immediately — dropped Task 3 (entry intro, left in-progress) and Task 8 (advisory) from this round rather than risk landing something half-verified right at the deadline, and focused the remaining time on getting everything already built actually live rather than adding more scope.
- The external collaborator's clone turned out reachable directly on this filesystem (`pick-and-lock-beyond-core-loop/`, a full separate clone with its own worktrees, not the stray leftover it was mistaken for earlier) — merged Tasks 5, 2, 6, and 7 directly from there instead of relaying push/pull instructions back and forth, cutting a full round-trip out of every merge.
- Merged Task 5 (`00f170b`, conversational multi-draft poll authoring) — clean fast-forward, 19/19 tests, matches the spec's multi-draft revision exactly (`poll_draft_updates` as a list, room-level constraints applied to every open draft, confirm-then-create gating).
- Merged Task 6 (Places rating enrichment) and Task 7 (hide manual form) — both fully disjoint from everything else, clean merges, no conflicts.
- Merged Task 2 (Tier 1 passive-intent classifier) — one real conflict in `service.ts` against Task 5's rewrite, resolved via `--conflict=diff3`: both sides added a pure new import line, combined directly. Found and fixed a real but expected issue during verification: the classifier's tests were written before Task 5 added `wants_poll`/`poll_draft_updates`/`cold_start_ideas`/`confirm_create` to `ModeratorResult`, so the shared test-mock default lacked them and crashed spreading `undefined` — added the missing fields to the shared mock (`87d4762`), not a design bug, a fork-timing gap between two branches built from a common ancestor.
- **Discovered mid-session that PR #29's private-decision-engine schema had never actually been published to Maincloud** — only merged into the code. Publishing today's `bot_poll_draft` table therefore also carried that backlog: two new columns on the existing `private_room` table (`decision_status`, `locked_choice_id`, both properly defaulted). SpacetimeDB still flagged the combined migration as breaking (disconnects live clients) despite the defaults — confirmed with the owner that nobody was actively connected at publish time, then published with `--yes=break-clients`.
- **Found and fixed a real gap during post-publish verification**: the bot-service had no reconnect logic — `onDisconnect` only logged, so the forced client disconnect from the breaking migration left the bot-service process alive but permanently disconnected from Maincloud, silently doing nothing. Recovered live with a manual `render restart`, then fixed the underlying gap directly since it was small: `RoomBotService` now rebuilds its connection on any unexpected disconnect after a 5-second delay (skipped when `stop()` was called intentionally). Two new tests cover both paths. 28/28 bot-service tests pass, `tsc --noEmit` clean.
- Redeployed the client to Vercel (`vercel --prod`) to ship Task 7's UI change; confirmed live at `pick-and-lock.vercel.app` (200, fresh build). Render auto-deployed the bot-service on every push per its existing Blueprint; confirmed the final commit (`87d4762`) reached `"status": "live"` before restarting it.
- Closed GitHub issues #1, #4, #10 (stale — merged earlier this session but never closed), and #32/#33/#34/#35 as each task landed.
...
- Rewrote `README.md` with an accurate account of what's shipped and a synthesized future-scope section (multi-agent collaboration, external link ingestion, location/route-aware suggestions).
- **Diagnosed a post-freeze live bug report** ("chat shows no messages after sending one"): root-caused to a stale browser-local SpacetimeDB identity that had survived two same-day breaking migrations, whose `my_room_chat` view silently dropped several of its room memberships with no error. Confirmed via a standalone Node script (same bindings, fresh identity) that the server-side view and reducer were both correct; confirmed the fix (clearing the stale `pick-and-lock-spacetime-token` localStorage key) restores full chat history immediately. No application code change was needed. See `docs/lessons.md` for the full diagnostic trail.
- Replaced the bare "Connecting to the room..." text with a branded loading screen, and added an empty-chat hint pointing first-time visitors to `@agent`.
- Reordered the room layout per explicit request: title full-width at the top, chat moved to the primary/wide column (and made taller), activities moved to the top of the secondary column, QR share code moved under the shared-context panel.
- Fixed the chat input rendering at browser-default width (~204px) instead of filling its row: the input's `flex:1` had no effect because its immediate parent (`.room-chat-input-wrapper`, added for the mention popup) wasn't a flex container. Fixed with `width:100%` on the input.
- Added a "Share room" button using the Web Share API (`navigator.share`), surfacing the OS-native share sheet (WhatsApp, Gmail, etc.) on Windows, Mac, Android, and iOS, falling back to the existing copy-link behavior where unsupported.

# 2026-09-07 — Root-caused a full bot outage (read-only, no code changed under freeze)

- Diagnosed why the bot stopped answering ~20 minutes before the freeze deadline: the `Friend.email` republish (`e5eb434`) was run as plain `spacetime publish` without the `BOT_IDENTITY=<id>` prefix every prior bot-enabling publish used. `BOT_IDENTITY` is a Rust compile-time env var (`option_env!`) never persisted anywhere in the repo, so that rebuild silently reset it to `None`, and `require_bot()` now rejects every bot-authenticated reducer call in every room, old and new, with zero error surfaced (swallowed by `service.ts`'s `.catch(() => undefined)`).
- Ruled out three other hypotheses before landing on this one: RLS-blocked plan visibility (disproved — `Plan` table is public, confirmed via source and a fresh-identity subscription test), bot identity/token mismatch (disproved — same identity had joined 6 rooms successfully before the republish), and `render restart` not taking effect (disproved — three restarts this session each produced a fresh, verified reconnect cycle with no change in outcome).
- No code, schema, or deployment was touched — investigation used only `spacetime sql` queries, Render log inspection, and standalone read-only diagnostic scripts, per the active code freeze. Full incident detail in `docs/lessons.md`; current-state summary in `docs/status.md`; remediation plan (not yet executed, pending owner go-ahead) in `docs/plan.md`.
- Drafted two LinkedIn posts (`docs/linkedin-posts.md`) and a personal LinkedIn banner/headline for the owner (design and copywriting only, no app code touched).

# 2026-09-07 — Bot authorization restored live; genuine bot reply still unverified (OpenAI emptyResult)

- Republished current `main` HEAD with `BOT_IDENTITY=<existing bot identity hex>` per the plan block (owner go-ahead, `--yes=remote`, no table/column changes), then `render restart`. **Auth fix confirmed against the live DB**, not just "it published": bot connected 16:17:22 UTC; `require_bot()`-gated `send_bot_message` wrote "AI Concierge" intros into rooms 4–30 at 16:17:24 and `advanceBotWatermark` moved room 14's watermark 59→97→100 — `require_bot()` authorizes the bot identity end-to-end again. Rooms 11/13/14/16/21/23 `Friend` rows unchanged.
- **Blocked on the task's done-criterion (genuine SQL-verified bot reply).** Every OpenAI call since reconnect is HTTP 200 with null/unparseable `message.content`, falling to `emptyResult` (`api/bot-service/openai.ts:138/:165`), logged as `reply=no ideas=0` on direct-address and silence-timeout triggers including a natural decision-focused `@sorted` message at 16:52:12. Startup sweep (16:17:38) hit the 6s timeout on all 12 rooms; after the service's auto-restart at 16:41:37 most rooms processed (still `reply=no`), rooms 11/21 still aborting. Separate bot-service/OpenAI-side issue, not the auth bug. Owner declined the local response-shape probe (needs the key); cause of `emptyResult` unconfirmed. 6s `fetchWithRetry` timeout in `api/bot-service/http.ts` is the `AbortError` source.
- Docs updated: `docs/status.md`, `docs/plan.md` (marked executed, done-criterion unmet). No application code or schema touched.

# 2026-09-07 — Bot reply verified live; three bot-service bugs root-caused and fixed

- **Done-criterion met: genuine SQL-verified bot reply.** At 17:51:10 UTC room 14 message 142 (`AI Concierge`, `is_bot=true`) — a substantive, decision-relevant reply to a direct-address `@sorted` message (141, 17:51:05). Rooms 11/13/14/16/21/23 `Friend` rows confirmed unchanged post-fix. See `docs/status.md` for full evidence.
- **Bug 1 — `gpt-5-nano` reasoning burns the token cap, returns empty content.** `api/bot-service/openai.ts` (`1f77cb0`). Diagnosed by adding response-body logging; the logs showed `finish_reason: "length"`, `reasoning_tokens === max_completion_tokens`, `message.content: ""` — a documented gpt-5-nano behavior where without `reasoning_effort` the model spends its whole `max_completion_tokens` on invisible reasoning and emits nothing output. Fix: `reasoning_effort: "minimal"` + raised OpenAI `fetchWithRetry` timeout to 30s (this also eliminated the 6s `AbortError`). Also added diagnostic logging at the `emptyResult` fallbacks.
- **Bug 2 — reply length validation cross-layer mismatch.** `api/bot-service/openai.ts` (`baada50`). `sanitizeReply` sliced to 500 *characters* (JS), but `send_bot_message` (`lib.rs:1402`) validates `body.len() > 500` *bytes* (Rust) → `SenderError: Bot message is invalid` on multibyte replies. Fix: byte-aware `truncateToBytes` via `TextEncoder`.
- **Bug 3 — `recordPreference` hallucinated friend_id aborts the batch.** `api/bot-service/service.ts` (`8bbfb23`). The model emits `extracted_preferences` with friend_ids from a roster it never receives; the reducer rejects with `Member not found`, which previously aborted `processRoomUnsafe` before the reply could send. Fix: wrapped `recordPreference` in non-fatal `.catch()` (logged, reply still sends). Also added `bot.pid` to `.gitignore` (`d939992`) after accidentally committing a local runtime artifact (`a6dce86` removed it).
- **Known follow-up:** preferences still aren't recorded because the model hallucinates friend_ids — pass the real member roster (`my_room_members` → friend ID/name) in the user content JSON so the model emits valid IDs.
